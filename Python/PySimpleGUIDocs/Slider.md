...
slider's range (min value, max value)
...
the smallest amount the slider can be moved

int or float

tick_interval

how often a visible tick should be shown next to slider

str

orientation

'horizontal' or 'vertical' ('h' or 'v' also work)

bool

disable_number_display

if True no number will be displayed by the Slider Element
...
relief style. Use constants - RELIEF_RAISED RELIEF_SUNKEN RELIEF_FLAT RELIEF_RIDGE RELIEF_GROOVE RELIEF_SOLID
...
If True then moving the slider will generate an Event
...
(l=length chars/rows, w=width pixels)
...
color of slider's background

str

text_color

color of the slider's text
